...
'''
x_m = np.round(np.arange(mlist[0], mlist[1]+(mlist[3]),mlist[3]), 5)
x_z = np.round(np.arange(zlist[0], zlist[1]+zlist[3], zlist[3]), 5)
#x_m = np.round(np.arange(mlist[0], mlist[1]+(mlist[3]),mlist[3]), 2)
t1 = np.arange(0, nt)


x_z_plot = x_z[:sample_full.shape[1]] + (zlist[3]/2)
x_m_plot = x_m[:sample_full.shape[3]] + (mlist[3]/2)
x_t = np.arange(0,nt+1)
'''
...
def function_plot(samples,axis_label,file_name):
   if axis_label == 'z' :
      a1,a2,a3 = 2,3,0
      x = x_z_plot
      x_data = x_z_data
   ...
   plt.plot(x,np.sum(like_stacked, axis=(a1-1, a2-1)), color='C0', zorder=10,label='Stacked Likelihood')
   ...
   #plt.plot(x,truth/np.sum(truth), color='C2', zorder=20)
   '''
   if axis_label == 'z' :
      plt.plot(x[1:sample_plot.shape[1]-1]+(space/2),np.convolve(np.mean(sample_plot, axis=0),np.ones((3,))/3, mode='valid'),color='C9')
      plt.plot(x[2:sample_plot.shape[1]-2]+(space/2),np.convolve(np.mean(sample_plot, axis=0),np.ones((5,))/5, mode='valid'),color='C6')
   '''
   if axis_label == 'z':
...
#print(weights.shape)
function_plot(sample_full,'z','Full_Survey_z.png')
print('z',flush=True)
function_plot(sample_full,'m','Full_Survey_m.png')
print('m',flush=True)
function_plot(sample_full,'t','Full_Survey_t.png')
...
for i in range(sample_full.shape[3]):
    ...
    ax_z[row, col].plot(x_z_plot, like_stacked[:, :, i].sum(axis=(1)), color='C0', zorder=10,label='Staked Likelihood')
    ...
    ax_t[row, col].plot(x_t, like_stacked[:,:, i].sum(axis=(0)), color='C0',zorder=10,label='Stacked Likelihood')
...
for i in range(sample_full.shape[2]):
    ...
    #ax_tz[row, col].plot(x_z_plot, like_stacked[:, i, :].sum(axis=(1)), color='C0',zorder=10)
    ax_tz[row, col].plot(x_z_plot, np.mean(sample_full[:, :, i, :].sum(axis=(2)), axis=0), 'C3',zorder=5)
    ...
    #ax_tz[row,col].plot(x_z_plot,truth/np.sum(truth),color='C2',zorder=20,label='t={}'.format(x_t[i]))
...
DIR_folder = '/rds/general/user/gk1513/home/SCRATCH/cx2-scratch2/Photo_Z_3D/DATA/Nz_DIR_Mean'
DIR_names = ['0.1t0.3', '0.3t0.5', '0.5t0.7', '0.7t0.9', '0.9t1.2']

SOM_folder = '/rds/general/user/gk1513/home/SCRATCH/cx2-scratch2/Photo_Z_3D/DATA/kids_1000/Nz_SOM'



def function_plot_tomo(samples,ax,row,col,axis_label,i,like_stacked):
   like_stacked1 = like_stacked
   if axis_label == 'z' :
      a1,a2,a3 = 2,3,0
      x = x_z_plot
      x_data = x_z_data
      
      nzdir = np.loadtxt(fname='{}/{}.asc'.format(DIR_folder,DIR_names[i]),delimiter=' ')
      nzdir = nzdir[:60,:]
      nz_avg = nzdir[:,1]
      nz_min = nz_avg - nzdir[:,2]
      nz_max = nz_avg + nzdir[:,2]
      ax[row,col].fill_between(nzdir[:,0], nz_min, nz_max, color='C8', alpha=0.3,label='DIR')
      #nzsom = np.loadtxt(fname='{}/Tomo_{}.asc'.format(SOM_folder,i+1),delimiter=' ')
      #nzsom[:,1] /= 0.05*np.sum(nzsom[:,1])
      #ax[row,col].plot(nzsom[:,0],nzsom[:,1],color='C2',label='SOM')
   elif axis_label == 'm' :
      a1,a2,a3 = 1,2,2
      x = x_m_plot
      x_data = x_m_data
   ...
   if axis_label == 'z' :
      samples /= 0.05
      like_stacked1 /= 0.05
      output = np.mean(np.sum(samples, axis=(a1, a2)),axis=0)
      output = np.column_stack((x,output))
      np.savetxt('{}/plots/Nz_DIR/Nz_DIR_Mean/Nz_DIR_z{}.asc'.format(output_folder,DIR_names[i]),output)
      output = np.column_stack((x,np.sum(samples, axis=(a1, a2)).T))
      np.savetxt('{}/plots/Nz_DIR/Nz_DIR_Bootstrap/Nz_z{}_FULL_DIR.asc'.format(output_folder,DIR_names[i]),output)
   ...
   ax[row,col].plot(x,np.sum(like_stacked1, axis=(a1-1, a2-1)), color='C0', zorder=10,label='Stacked Likelihood')
   ...
   ax[row,col].fill_between(x[:samples.shape[a3+1]]+(space/2),sigma_low,sigma_high,alpha=0.4,color='C3',zorder=1)
   ax[row, col].fill_between(x[:samples.shape[a3+1]]+(space/2),
                             sigma_lowlow, sigma_highhigh, alpha=0.15, color='C3',zorder=1)
   #ax[row, col].plot(x[:samples.shape[a3+1]]+(space/2),
   #                  np.sum(stacked_likelihood, axis=(a1-1, a2-1)), color='C0',zorder=10)
   ax[row, col].plot(x[:samples.shape[a3+1]]+(space/2),
                     mean, color='C3',zorder=5)
   ax[row,col].hist(data[:,a3],bins=x
            ,range=[x[0],x[-1]],histtype='step',density=False,color='C2',weights=weight,zorder=20)
   '''
...
for i in reversed(range(5)):
    ...
    function_plot_tomo(sample_tomo,axz,row,col,'z',i,like_stacked)
    function_plot_tomo(sample_tomo,axt,row,col,'t',i,like_stacked)
    function_plot_tomo(sample_tomo,axm,row,col,'m',i,like_stacked)
    sample_tomo = None
    like_stacked = None
    data_tomo = None
    weight_tomo = None
